...
void tickFct2() {
	...
	switch(state2) {
		case display:
			PORTA = temp2; //want to be 0x80
			PORTB = temp1; //want to be 0xE3
			//PORTB = temp1;
			//PORTA = mover_level + (temp2 >> 1);
			break;
		default:
			break;
	}
...
*/
/*
enum Statess {win, lose, reset} state2;
void endFct() {
	switch(state2) {
		case win:
			if((~PIND & 0x01) == 0x01) {
